...
\section{Multi type assignment}
\label{ref:ModelElementAssignMulti}

\begin{wrapfigure}{l}{2.5cm}
\vspace{-22pt}
\includegraphics[width=2cm]{imageModelElementAssignMulti.png}
\vspace{-22pt}
\end{wrapfigure}

The multi type assignment element assigns a new client type (for the statistics) to the clients who are passing this element.
Different client types can be assigned depending on configurable conditions.

\subsection*{Settings}

Any number of client types can be specified and for each a condition as to when this assignment should be applied.
...
If the only purpose of the time-dependent check is to always release waiting clients after a specified maximum time
...
The recording can optionally be differentiated by client type and time interval.
...
Interval-based recording can also be activated.
In this case, the number and average value of the values recorded
in the interval is recorded for each interval (the length of which can be set)
for each discrete-time value. Recording begins at the end of the warm-up phase.
