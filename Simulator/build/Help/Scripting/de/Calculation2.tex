...
\item
\cmd{TNowTime()}:\\
Liefert die aktuelle Simulationszeit innerhalb des Tages als Sekundenwert, d.h. einen Wert zwischen 0 (einschließlich) und 86400 (ausschließlich).

\item
\cmd{TNowDay()}:\\
Liefert den aktuellen Simulationstag, d.h. eine Ganzzahl von 0 oder einem höheren Wert.
...
\item
\cmd{Zähler(id;index)} oder \cmd{Counter(id;index)} oder \cmd{Value(id;index)} oder \cmd{Wert(id;index)}:\\
Liefert den Wert des Zählers mit 1-basiertem Index \cm{index} an der Mehfachzählerstation \cm{id}.

\item
\cmd{Anteil(id;index)} oder \cmd{Part(id;index)}:\\
Liefert den Anteil des Zählerwertes des Zählers mit dem 1-basierend Index \cm{index} innerhalb der Zählergruppe, in der er sich befindet an der Mehrfachzählerstation \cm{id}.
...
\item
\cmd{resource\_count\_average(id)} oder \cmd{resource\_capacity\_average(id)} oder \cmd{MR\_average(id)}:\\
Liefert die Anzahl der im Mittel vorhandenen Bediener in der angegebenen Ressource.
...
\item
\cmd{resource\_median(id)} oder \cmd{utilization\_median(id)}:\\
Liefert den Median der Anzahl an belegten Bedienern in der angegebenen Ressource.
  
\item
\cmd{resource\_quantil(id;p)} oder \cmd{utilization\_quantil(id;p)}:\\
Liefert das Quantil zur Wahrscheinlichkeit p der Anzahl an belegten Bedienern in der angegebenen Ressource.
...
\item
\cmd{resource\_count\_average()} oder \cmd{resource\_capacity\_average()} oder \cmd{MR\_average()}:\\
...
Liefert den Median der Anzahl an belegten Transportern in der angegebenen Transportergruppe.
  
\item
\cmd{transporter\_quantil(id;p)} oder \cmd{transporter\_utilization\_quantil(id;p)}:\\
Liefert das Quantil zur Wahrscheinlichkeit p der Anzahl an belegten Transportern in der angegebenen Transportergruppe.
...
\item
\cmd{LetzteWartezeittoleranz()} oder \cmd{LastWaitingTimeTolerance()}:\\
Liefert die zuletzt für diesen Kunden an einer Bedienstation berechnete Wartezeittoleranz. Wurde für den Kunden noch nie eine Wartezeittoleranz berechnet, so liefert die Funktion 0.
