...
\item
\cmd{pi}: Liefert den Durchmesser des Einheitskreises $\tau=2\cdot\pi$. Es gilt
...
\item
\cmd{NormalDist(a;b)}:
Erzeugt eine nichtnegative Zufallszahl gemäß der Verteilung.
...
\section{Inverse Gamma-Verteilung mit Mittelwert \texorpdfstring{$a$}{a} und Standardabweichung \texorpdfstring{$b$}{b}}
...
\section{Inverse Gauß-Verteilung mit Mittelwert \texorpdfstring{$a$}{a} und Standardabweichung \texorpdfstring{$b$}{b}}
...
\item
\cmd{IrwinHallDistDirect(x;a;0)}:
Berechnet die Dichte an der Stelle $x$.

\item
\cmd{IrwinHallDistDirect(x;a;1)}:
Berechnet den Wert der Verteilungsfunktion an der Stelle $x$.

\item
\cmd{IrwinHallDistDirect(a)}:
Erzeugt eine Zufallszahl gemäß der Verteilung.
...
\section{Kontinuierliche Bernoulli-Verteilung über \texorpdfstring{$[a;b]$}{[a;b]} mit Parameter \texorpdfstring{$\lambda$}{lambda}}
...
\section{Log-Laplace-Verteilung mit Parameter \texorpdfstring{$c$}{c} und Startpunkt \texorpdfstring{$s$}{s}}
...
\section{Phasentypverteilung}

\cmd{PhaseTypeDistRandom(s...;alpha...)}\\
Erzeugt eine Zufallszahl gemäß  Phasentypverteilung, die über die Matrix s (zeilenweise angegeben) und dem Vektor alpha parametrisiert ist.
