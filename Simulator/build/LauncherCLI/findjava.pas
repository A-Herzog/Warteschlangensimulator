...
{ TFindJava }
 ...
 TFindJava=class
  ...
  published
    ...
    property SimJar : String read fSimJar;
...
begin
  ...
  fSimJar:=fOwnPath+'Simulator.jar';

  fIsSimFound:=FileExists(fSimJar);
end;
...
begin
  ...
  JavaMainFile1:=FullPath+'java.exe';
  JavaMainFile2:=FullPath+'bin'+PathDelim+'java.exe';
...
begin
  ...
  If IsJavaPath(OwnPath+'jdk') then Exit;

  { 1b- in .\jre directory (JRE Installed with application) }
  If IsJavaPath(OwnPath+'jre') then Exit;
