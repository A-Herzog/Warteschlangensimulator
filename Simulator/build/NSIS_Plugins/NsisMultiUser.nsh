...
!macro MULTIUSER_FUNCTION_INSTALLMODEPAGE UNINSTALLER_PREFIX UNINSTALLER_FUNCPREFIX
		...
		${else}
			!if "${UNINSTALLER_FUNCPREFIX}" == ""
				...
				${else}
					...
					ReadRegStr $INSTDIR HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders" "AppData"
					${if} "$INSTDIR" == ""
	...
	Function ${UNINSTALLER_FUNCPREFIX}MultiUser.InitChecks
		...
		; PATCH
		IfSilent 0 noSilentReplaceForOption
		${if} $CmdLineInstallMode == ""
			StrCpy $CmdLineInstallMode "CurrentUser"
		${endif}
		noSilentReplaceForOption:
...
!macro MULTIUSER_GetCurrentUserString VAR
	...
	!if ${MULTIUSER_INSTALLMODE_ALLOW_BOTH_INSTALLATIONS} <> 0
		${if} $MultiUser.InstallMode == "CurrentUser"
			...
			; PATCH
			; StrCpy ${VAR} " ($(MULTIUSER_CURRENT_USER_UNINSTALL))"
			StrCpy ${VAR} " (current user)" ; Reg key always with English translation. Otherwise install/uninstall using different languages won't work
		${endif}
	!endif
!macroend
...
!macro MULTIUSER_RegistryAddInstallInfo
	...
	${else}
		WriteRegStr SHCTX "${MULTIUSER_INSTALLMODE_UNINSTALL_REGISTRY_KEY_PATH}$0" "DisplayName" "${MULTIUSER_INSTALLMODE_DISPLAYNAME} ($(MULTIUSER_CURRENT_USER_UNINSTALL))" ; "add/remove programs" will show if installation is per-user
