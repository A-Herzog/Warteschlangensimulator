...
public class RunElementSourceTable extends RunElementSourceExtern {
	 ...
	 */
	private String loadTableFile(final File file, final String setup, final List<String> clientTypes, final boolean numbersAreDistances, final boolean bottomUp, final RunModel runModel) {
		final Table table=new Table();
		if (!table.load(file)) return String.format(Language.tr("Simulation.Creator.TableFile.LoadFailed"),file.toString(),id);
		return loadTable(table,setup,clientTypes,numbersAreDistances,bottomUp,runModel.scaleToSimTime);
	}
	...
	public Object build(final EditModel editModel, final RunModel runModel, final ModelElement element, final ModelElementSub parent, final boolean testOnly) {
		...
		/* Tabelle verarbeiten */
		if (!testOnly) {
			error=source.loadTableFile(tableFile,setup,clientTypes,numbersAreDistances,sourceElement.isReadBottomUp(),runModel);
			if (error!=null) return error;
		}
