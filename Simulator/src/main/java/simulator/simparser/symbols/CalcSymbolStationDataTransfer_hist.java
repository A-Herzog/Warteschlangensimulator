 ...
 * Liefert den Anteil der Kunden, für den die Transferzeit an Station id (1. Parameter) mehr als timeA (2. Parameter) und höchstens timeB (3. Parameter) Sekunden gedauert hat.<br>
 * Im Falle von zwei Parametern:<br>
 * Liefert den Anteil der Kunden, für den die Transferzeit an Station id (1. Parameter) time (2. Parameter) Sekunden gedauert hat.
...
public class CalcSymbolStationDataTransfer_hist extends CalcSymbolStationDataHistogram {
	 ...
	 */
	public CalcSymbolStationDataTransfer_hist() {
		/*
	...
	protected DataDistributionImpl getDistributionForClientType(final String name) {
		if (lastClientType==null || !name.equals(lastClientType)) {
			lastIndicator=((StatisticsDataPerformanceIndicator)getSimData().statistics.clientsTransferTimes.get(name));
	...
	protected double getDistributionSum(RunElementData data) {
		...
		return data.statisticTransfer.getCount();
	}
}
