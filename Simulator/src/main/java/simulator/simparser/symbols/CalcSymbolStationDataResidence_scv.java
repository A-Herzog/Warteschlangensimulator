 ...
 * Liefert den quadrierten Variationskoeffizienten der Verweilzeiten über alle Kunden.
...
public class CalcSymbolStationDataResidence_scv extends CalcSymbolStationData {
	...
	protected double calcStationClient(final RunElementData data, final int clientTypeIndex) {
		...
		final double cv=data.statisticResidenceByClientType[clientTypeIndex].getMean();
		return cv*cv;
	}
}
