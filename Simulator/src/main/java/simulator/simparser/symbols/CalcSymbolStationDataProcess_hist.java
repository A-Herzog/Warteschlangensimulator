...
public class CalcSymbolStationDataProcess_hist extends CalcSymbolStationDataHistogram {
	...
	protected DataDistributionImpl getDistributionForClientType(final String name) {
		if (lastClientType==null || !name.equals(lastClientType)) {
			lastIndicator=((StatisticsDataPerformanceIndicator)getSimData().statistics.clientsProcessingTimes.get(name));
	...
	protected double getDistributionSum(RunElementData data) {
		if (data.statisticProcess==null) return 0.0;
		return data.statisticProcess.getCount();
	}
}
