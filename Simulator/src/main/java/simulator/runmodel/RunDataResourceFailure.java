...
public class RunDataResourceFailure {
	...
	public boolean testStartPause(final SimulationData simData, final RunDataResource resource, final RunDataResourceOperatorFull operator, final int failureIndex) {
		...
		if (pauseStartTime>=0 && pauseStartTime<=simData.currentTime) {
			...
			if (simData.loggingActive && simData.logInfoSystem) simData.logEventExecution(Language.tr("Simulation.Log.ResourceFailure"),String.format(Language.tr("Simulation.Log.ResourceFailure.Scheduled"),name,operator.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
		...
		if (failureMode==ModelResourceFailure.FailureMode.FAILURE_BY_NUMBER && servedCount>=failureNumber) {
			...
			servedCount=0;
			long downTime=getDownTime(simData,resource.getName());
			if (simData.loggingActive && simData.logInfoSystem) simData.logEventExecution(Language.tr("Simulation.Log.ResourceFailure"),String.format(Language.tr("Simulation.Log.ResourceFailure.WorkCount"),name,operator.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
			operator.onlineAgainAt=simData.currentTime+downTime;
			scheduleResourceCheckEvent(simData,operator.onlineAgainAt);
			return true;
		}
		...
		if (failureMode==ModelResourceFailure.FailureMode.FAILURE_BY_AVAILABLE_TIME && simData.currentTime>=operator.availableStartTime+failureTime) {
			resource.startDownTime(simData.currentTime);
			long downTime=getDownTime(simData,resource.getName());
			if (simData.loggingActive && simData.logInfoSystem) simData.logEventExecution(Language.tr("Simulation.Log.ResourceFailure"),String.format(Language.tr("Simulation.Log.ResourceFailure.AvailableTime"),name,operator.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
			operator.onlineAgainAt=simData.currentTime+downTime;
			scheduleResourceCheckEvent(simData,operator.onlineAgainAt);
			return true;
		}
		...
		if (failureMode==ModelResourceFailure.FailureMode.FAILURE_BY_WORKING_TIME && servedTime>failureTime) {
			...
			servedTime=0;
			long downTime=getDownTime(simData,resource.getName());
			if (simData.loggingActive && simData.logInfoSystem) simData.logEventExecution(Language.tr("Simulation.Log.ResourceFailure"),String.format(Language.tr("Simulation.Log.ResourceFailure.WorkTime"),name,operator.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
			operator.onlineAgainAt=simData.currentTime+downTime;
			scheduleResourceCheckEvent(simData,operator.onlineAgainAt);
			return true;
		}
