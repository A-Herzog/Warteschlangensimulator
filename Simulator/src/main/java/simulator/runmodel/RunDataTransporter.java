...
public class RunDataTransporter {
	...
	public long moveTo(final int stationID, final int clientCount, final SimulationData simData) {
		...
		if (!initialMove) {
			/* Logging */
			if (simData.loggingActive && simData.logInfoSystem) simData.logEventExecution(Language.tr("Simulation.Log.Transporter"),-1,String.format(Language.tr("Simulation.Log.Transporter.Move"),"\""+list.type[type]+"\"("+hashCode()+")",position,stationID,TimeTools.formatExactSystemTime(transferTime)));
	...
	public void arrival(final SimulationData simData, final long arrivalTime) {
		...
		/* Logging */
		if (simData.loggingActive && simData.logInfoSystem) simData.logEventExecution(Language.tr("Simulation.Log.Transporter"),-1,String.format(Language.tr("Simulation.Log.Transporter.Arrival"),"\""+list.type[type]+"\"("+hashCode()+")",position));
