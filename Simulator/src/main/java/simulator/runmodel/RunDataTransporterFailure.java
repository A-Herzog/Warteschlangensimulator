...
public class RunDataTransporterFailure {
	...
	public boolean testStartPause(final SimulationData simData, final RunDataTransporter transporter) {
		...
		if (pauseStartTime>0 && pauseStartTime<=simData.currentTime) {
			...
			if (simData.loggingActive && simData.loggingIDs==null) simData.logEventExecution(Language.tr("Simulation.Log.TransporterFailure"),String.format(Language.tr("Simulation.Log.TransporterFailure.Scheduled"),list.type[transporter.type],transporter.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
		...
		if (failureMode==ModelTransporterFailure.FailureMode.FAILURE_BY_NUMBER && servedCount>=failureNumber) {
			transporter.startDownTime(simData);
			final long downTime=getDownTime(simData,simData.runModel.transportersTemplate.type[transporter.type]);
			if (simData.loggingActive && simData.loggingIDs==null) simData.logEventExecution(Language.tr("Simulation.Log.TransporterFailure"),String.format(Language.tr("Simulation.Log.TransporterFailure.WorkCount"),list.type[transporter.type],transporter.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
			transporter.onlineAgainAt=simData.currentTime+downTime;
			scheduleTransporterPauseEndEvent(simData,transporter);
			servedCount=0;
			return true;
		...
		if (failureMode==ModelTransporterFailure.FailureMode.FAILURE_BY_DISTANCE && servedDistance>=failureDistance) {
			transporter.startDownTime(simData);
			final long downTime=getDownTime(simData,simData.runModel.transportersTemplate.type[transporter.type]);
			if (simData.loggingActive && simData.loggingIDs==null) simData.logEventExecution(Language.tr("Simulation.Log.TransporterFailure"),String.format(Language.tr("Simulation.Log.TransporterFailure.Distance"),list.type[transporter.type],transporter.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
			transporter.onlineAgainAt=simData.currentTime+downTime;
			scheduleTransporterPauseEndEvent(simData,transporter);
			servedDistance=0;
			return true;
		...
		if (failureMode==ModelTransporterFailure.FailureMode.FAILURE_BY_WORKING_TIME && servedTime>failureTime) {
			...
			if (simData.loggingActive && simData.loggingIDs==null) simData.logEventExecution(Language.tr("Simulation.Log.TransporterFailure"),String.format(Language.tr("Simulation.Log.TransporterFailure.WorkTime"),list.type[transporter.type],transporter.index+1,SimData.formatSimTime(simData.currentTime),SimData.formatSimTime(downTime)));
			transporter.onlineAgainAt=simData.currentTime+downTime;
			scheduleTransporterPauseEndEvent(simData,transporter);
			servedTime=0;
			return true;
