...
public class RunDataResourceOperatorFull extends RunDataResourceOperator {
	...
	public boolean isAvailableOrWorking(final RunDataResource resource, final SimulationData simData) {
		...
		if (onlineAgainAt>0) {
			...
			if (simData.loggingActive) simData.logEventExecution(Language.tr("Simulation.Log.ResourceFailure"),String.format(Language.tr("Simulation.Log.ResourceFailure.EndOfDownTime"),name,index+1,SimData.formatSimTime(onlineAgainAt)));
