...
public class ModelElementDecoration extends ModelElementPosition implements ElementWithAnimationEditOptions {
	...
	protected final boolean isNearLine(final Point p, final Point q, final Point point, final double zoom, final int lineWidth) {
		...
		if (Math.abs(x.x-y.x)>lineWidth/2.0*zoom+MAX_POINT_DELTA || Math.abs(x.y-y.y)>lineWidth/2.0*zoom+MAX_POINT_DELTA) return false; /* Abstand von Gerade zu groß? */
	...
	protected String loadProperty(final String name, final String content, final Element node) {
		...
		if (Language.trAll("Surface.XML.Color",name) && !content.isBlank()) {
